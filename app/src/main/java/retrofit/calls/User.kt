...
object User {
    ...
    suspend fun subscriptions(onSuccess: (List<Subscription>) -> Unit, onError: ((Exception) -> Unit)? = null, showLoading: Boolean = true) {
        var loadingDialog: LoadingDialog? = null
        if (showLoading) {
            loadingDialog = LoadingDialog(App.ACTIVITY, App.ACTIVITY.getString(R.string.api_user_subscriptions))
            loadingDialog.show()
        }
        try {
